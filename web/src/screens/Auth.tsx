...
function SocialBtn({ icon, label, tk, onClick }: { icon: React.ReactNode; label: string; onClick: () => void; tk: { inputBg: string; inputBorder: string; sub: string; card: string; line: string; text: string } }) {
  return (
    <button type="button" onClick={onClick} style={{
      flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
      padding: '14px', borderRadius: 13, border: `1.5px solid ${tk.line}`,
      background: tk.card, color: tk.text,
      fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'Sora,system-ui',
...
export default function Auth() {
  ...
  const { role } = useSearch({ from: '/auth' }) as { role: Role }

  const handleSubmit = () => {
    navigate({ to: role === 'provider' ? '/provider' : '/map' })
  }
      ...
      <div style={{ minHeight: '100vh', background: tk.bg, display: 'flex', flexDirection: 'column' }}>
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px' }}>
          <div style={{ width: '100%', maxWidth: 420, animation: 'fadeUp .32s ease' }}>
            ...
            <p style={{ fontSize: 14, color: tk.muted, marginBottom: 28, lineHeight: 1.6 }}>
              {role === 'customer' ? 'Zero fees for customers. Forever.' : 'Always $1 per booking. Keep everything else.'}
            </p>

            {/* Social login */}
            <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
              <SocialBtn label="Apple" tk={tk} onClick={handleSubmit} icon={
                <svg width="16" height="16" viewBox="0 0 16 16" fill={tk.sub}>
              ...
              <SocialBtn label="Google" tk={tk} onClick={handleSubmit} icon={
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <path d="M15.545 6.558a9.42 9.42 0 0 1 .139 1.626c0 2.434-.87 4.492-2.384 5.885h.002C11.978 15.292 10.158 16 8 16A8 8 0 1 1 8 0a7.689 7.689 0 0 1 5.352 2.082l-2.284 2.284A4.347 4.347 0 0 0 8 3.166c-2.087 0-3.86 1.408-4.492 3.304a4.792 4.792 0 0 0 0 3.063h.003c.635 1.893 2.405 3.301 4.492 3.301 1.078 0 2.004-.276 2.722-.764h-.003a3.702 3.702 0 0 0 1.599-2.431H8v-3.08h7.545z" fill={tk.sub}/>
                </svg>
              } />
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
              ...
              <span style={{ fontSize: 11, color: tk.muted, fontWeight: 600 }}>or continue with email</span>
              <div style={{ flex: 1, height: 1, background: tk.line }} />
            </div>

            {/* Tab toggle */}
            <div style={{ display: 'flex', gap: 6, marginBottom: 24, background: tk.inputBg, padding: 4, borderRadius: 12 }}>
              {(['login', 'signup'] as AuthMode[]).map(m => (
                <button type="button" key={m} onClick={() => setAuthMode(m)} style={{
                  flex: 1, padding: '10px', borderRadius: 9, border: 'none',
                  fontSize: 13, fontWeight: 700,
                  background: authMode === m ? tk.text : 'transparent',
                  color: authMode === m ? tk.bg : tk.muted,
            ...
            {authMode === 'signup' && (
              <p style={{ fontSize: 11, color: tk.muted, textAlign: 'center', marginTop: 16, lineHeight: 1.7 }}>
                By creating an account you agree to our{' '}
                <a href="#" onClick={e => e.preventDefault()} style={{ color: tk.sub, textDecoration: 'underline' }}>Terms of Service</a>
                {' '}and{' '}
                <a href="#" onClick={e => e.preventDefault()} style={{ color: tk.sub, textDecoration: 'underline' }}>Privacy Policy</a>.
              </p>
            )}
          </div>
        </div>
